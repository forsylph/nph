...
│       ├── loadNavigation()               # XML 로딩
│       ├── findAction()                   # Action 찾기
│       └── process()                      # 실행 흐름 제어
│
├── interceptor/                             # 인터셉터
│   ├── Interceptor.java                     # 인터페이스
│   │   └── doIntercept()
│   │
│   ├── LoginCheckInterceptor.java           # 로그인 체크
│   ├── ConverterInterceptor.java            # 데이터 변환
│   └── CommandInterceptor.java              # Command 실행
│
├── transaction/                             # 트랜잭션
│   ├── TxServiceUtil.java                     # 트랜잭션 유틸
│   │   ├── getNTxService()                    # 서비스 획득 (트랜잭션 시작)
│   │   ├── begin()                            # 트랜잭션 시작
│   │   ├── commit()                           # 커밋
│   │   └── rollback()                         # 롤백
│   │
│   └── DevonTransactionManager.java
│       └── ...
│
├── data/                                    # 데이터 처리
...
│   │   └── addColumn()                        # 컬럼 정의
│   │
│   └── LQueryService.java                     # XML Query 실행
│       ├── executeQuery()                     # 쿼리 실행
│       ├── executeUpdate()                    # UPDATE/INSERT/DELETE
│       └── executeBatch()                     # 배치 처리
│
├── util/                                    # 유틸리티
│   ├── LActionContext.java                  # ThreadLocal 컨텍스트
...
[Navigation Layer]
    │
    ▼ NavigationManager.findAction("SaveUser")
        │
...
[Interceptor Layer]
    ...
    ▼ Interceptor Chain
        ...
        ├── ConverterInterceptor.doIntercept()
        └── CommandInterceptor.doIntercept()
            │
            └── new SavePbhlCdCMD()
                │
[Command Layer]
...
| **Data** | `LQueryService` | XML Query 실행 |
| **Transaction** | `TxServiceUtil` | 트랜잭션 관리 |
| **Util** | `LActionContext` | ThreadLocal 컨텍스트 |
