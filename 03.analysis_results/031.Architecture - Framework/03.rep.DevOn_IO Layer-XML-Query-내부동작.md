...
> 주의: 이 문서는 내부 동작 복원/추정 문서를 포함합니다. 현재 백업셋에서는 `LQueryService` 실파일을 직접 확인하지 못했고, 업무 코드에서는 `LCommonDao(...).executeQuery()` 호출 흔적이 더 직접적으로 보입니다.
...
- ❌ iBatis 사용 안 함
- ❌ MyBatis 사용 안 함
- ✅ **DevOn 계열 XML Query 접근 구조**

```mermaid
flowchart LR
    A["업무 코드"] --> B["LCommonDao / XML Query 접근 계층"]
    B --> C["devonhome/xmlquery/*.xml"]
    C --> D["statement 선택"]
    D --> E["JDBC 실행"]
    E --> F["ResultSet"]
...
```
XML 파일 → XML Query 접근 계층 → JDBC 실행 → ResultSet → LMultiData
```
...
public class LoginPC {
    public LMultiData validateUser(LData input) {
        ...
        // 아래 호출명은 설명용 의사코드다.
        // 현재 백업셋에서는 업무 코드에서 LCommonDao(...).executeQuery() 패턴이 더 직접적으로 관찰된다.
        LMultiData result = runXmlQuery(
