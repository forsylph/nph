...
> 주의: 본 문서는 실제 `MiplatformConverter.java` 기반 분석이지만, 실행 흐름 예시 일부는 단순화되어 있습니다. 실제 변환 이후 DB 접근은 현재 백업셋 기준 `LQueryService`보다 `LCommonDao` 중심 업무 호출이 더 직접적으로 관찰됩니다.
...
```mermaid
flowchart LR
    A["MiPlatform Dataset"] --> B["MiplatformRequest"]
    B --> C["convertToLMultiDataWithJobType"]
    C --> D["LData / LMultiData"]
    D --> E["Command / PC / EC"]
    E --> F["convertToDataset"]
    F --> G["MiPlatform Dataset 응답"]
```
...
protected void beforeCatchService() {
    ...
    LMultiData inputData = MiplatformConverter.convertToLMultiDataWithJobType(ds_Input, sessionDs);
...
```mermaid
sequenceDiagram
    participant Client as MiPlatform Client
    participant Req as MiplatformRequest
    participant Conv as MiplatformConverter
    participant Biz as Business Logic
    participant Res as MiplatformResponse

    Client->>Req: Dataset 전송
    Req->>Conv: 수신 데이터 변환
    Conv->>Biz: LData / LMultiData 전달
    Biz->>Conv: 처리 결과 반환
    Conv->>Res: Dataset 재변환
    Res-->>Client: Dataset 응답
```
...
public void execute() {
    ...
    LMultiData input = MiplatformConverter.convertToLMultiDataWithJobType(inputDs, sessionDs);
