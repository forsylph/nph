...
| 운영 로그 | `NPH_HIS/devonhome/logs/20260304PMinfo.log`의 `HttpClientUtil Loaded Cert`, `Registered Handler: LST`, `Registered Handler: POLNET` |
...
`2026-03-04` 로그에서 아래가 직접 확인된다.

- `AsyncWorkContextListener Initializing context...`
- `HttpClientUtil Loaded Cert: ...`
- `Registered Handler: LST -> com.rest.external.lst.LstHandler`
- `Registered Handler: POLNET -> com.rest.external.polnet.PolNetHandler`

판단:
- `HttpClientUtil`은 단순 유틸 클래스가 아니라 사이트 기동 시점의 외부 연동 계층에 직접 참여한다.
- `LST`, `POLNET`은 문서상 연동 후보가 아니라 실제 핸들러 등록까지 확인된 운영 대상이다.
