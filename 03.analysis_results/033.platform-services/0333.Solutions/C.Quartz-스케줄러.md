...
이 문서는 `Quartz JAR 존재`와 `현행 배치 운영 구조`를 구분해서 읽어야 한다. 현재 코드베이스에서는 **Quartz 1.6.1 JAR 존재는 확인되지만, `org.quartz` API 직접 사용은 확인되지 않았다.** 반면 실제 운영 표면은 **DEVON Batch 컨테이너 + `BatchExecutor.cmd` + `batchMgr` UI + DB 메타데이터** 조합으로 읽는 것이 더 타당하다.
...
| 구성 요소 | 버전 | 설명 |
|----------|------|------|
| **Quartz** | 1.6.1 | 포함 JAR | 현재 코드베이스 기준 직접 사용 미확인 |
...
> 현재 확인 기준: `Quartz`는 포함 라이브러리로는 보이지만, 운영 스케줄 실체를 설명하는 1차 근거는 아니다. 이 장의 핵심은 `DEVON Batch 스케줄 운영 구조`다.
