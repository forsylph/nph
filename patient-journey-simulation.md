...
> **참고**: 위 화면 ID는 실제 코드베이스에서 확인된 예시를 포함하지만, 단계별 흐름에 배치된 액션/업무 연결은 설명용 단순화가 섞여 있습니다. 실제 화면 ID 패턴은 `{모듈}_{하위}_{5자리숫자}{M/P}` 형식입니다.
...
flowchart LR
    ...
    subgraph Server["DevOn Server"]
        ...
        Navi["Navigation<br/>az/bizcom/authNavi.xml"]
        Command["Command<br/>CheckLoginMiCMD"]
        PC["Process Component<br/>LoginIFPC / az.bizcom.LoginPC"]
        XML["XML Query/DAO<br/>인증 관련 조회"]
    end

    Login -.->|"/az/bizcom/authNavi/CheckLoginUser.mhi"| Servlet
    Servlet --> Navi
    Navi --> Command
    Command --> PC
    PC --> XML
...
> 아래 `Transaction/Navigation/Command/PC/XML Query` 명칭은 **실행 흐름 설명용 예시**입니다.
> 이 구간의 `RegisterOutpatient.mhi`, `RegisterOutpatientCMD`, `ReceiptPC`, `receipt.xml`은 현재 백업셋에서 1:1 실파일로 검증하지 않았습니다.

#### 1.4-1 실제 검증 사례

| 구분 | 실제 확인값 | 근거 |
|------|-------------|------|
| **Login Navigation** | `devonhome/navigation/mhi/az/bizcom/authNavi.xml` | `CheckLoginUser -> CheckLoginMiCMD` |
| **Outpatient Navigation** | `devonhome/navigation/mhi/md/opn/otptnrcrNavi.xml` | `SaveOtptMdcrCmpl -> SaveOtptMdcrCmplCMD` |
| **Inpatient Navigation** | `devonhome/navigation/mhi/md/ipn/admsnrcrNavi.xml` | `RegistAdmsEntc -> RegistAdmsEntcCMD` |
| **실제 CMD 예시** | `CheckLoginMiCMD`, `SaveOtptMdcrCmplCMD`, `RegistAdmsEntcCMD` | 소스 실존 확인 |

```java
// 실제 검증된 CMD 예시 1: 로그인
// authNavi.xml -> CheckLoginUser -> CheckLoginMiCMD
public void execute() throws Exception {
    ...
    if (userInfo.containsKey("usid")) {
        LData lResult = loginPC.doLogin(data);
        platformResponse.addDataset("OUT_DS1", (LData) lResult.get("OUT_DS1"));
    }
}

// 실제 검증된 CMD 예시 2: 외래 진료 완료 저장
// otptnrcrNavi.xml -> SaveOtptMdcrCmpl -> SaveOtptMdcrCmplCMD
public void execute() throws Exception {
    OtptMdcrPrprIFPC pc = (OtptMdcrPrprIFPC) TxServiceUtil.getTxService("md.opn.OtptMdcrPrprPC");
    LMultiData mData1 = getMultiDatasetWithJobType("ds_ApntAndThdyRcpnPt");
    int iResult = pc.saveOtptMdcrCmpl(mData1, data);
    platformResponse.setParam("nSaveCnt", iResult);
}
```
...
| 구분 | 클래스/파일 | 경로 |
|------|-------------|------|
| **Navigation** | `(예시) az/comn/receiptNavi.xml` | `devonhome/navigation/...` |
| **Command** | `(예시) RegisterOutpatientCMD` | `nph.his.{module}.{biz}.cmd` |
| **PC** | `(예시) ReceiptPC` | `nph.his.{module}.{biz}.pc` |
| **XML Query** | `(예시) receipt.xml` | `devonhome/xmlquery/{module}/{biz}/` |
...
> **참고**: 아래 화면 ID는 실제 코드베이스에서 확인된 예시입니다. 다만 화면 간 연결 순서와 업무 시나리오는 개념 설명을 위해 단순화되어 있습니다.
...
예시 (패턴 설명용):
- /az/bizcom/authNavi/CheckLoginUser.mhi
- /md/opn/otptnrcrNavi/SaveOtptMdcrCmpl.mhi
- /md/ipn/admsnrcrNavi/RegistAdmsEntc.mhi
- /az/bizcom/cmcdNavi/SaveDetail.mhi
```

> **참고**: 실제 코드에서는 `mhi` 폴더 아래에 네비게이션 파일이 위치합니다. 다만 위 액션명들은 패턴 설명용 예시이며, 현재 백업셋에서 전부 실존 확인된 것은 아닙니다.
...
| Navigation 경로 | his/{module}/... | `*.mhi` 요청 + `devonhome/navigation/...` 매핑 |
...
@검증됨: devonhome/navigation/mhi/md/opn/otptnrcrNavi.xml
@검증됨: devonhome/navigation/mhi/az/bizcom/authNavi.xml
